...
def compute_coverage_by_year(
    ...
    coverage = df.group_by("year").agg([
        pl.n_unique("symbol").alias("n_stocks")
    ]).sort("year")
    ...
    coverage = coverage.with_columns([
        pl.col("n_stocks").fill_null(0),
        pl.col("pct_complete").fill_null(0.0)
    ])
...
def format_coverage_table(coverage: pl.DataFrame) -> str:
    ...
    lines.append(f"**Min coverage:** {coverage['pct_complete'].min():.1f}% (year {coverage.filter(pl.col('pct_complete') == pl.col('pct_complete').min())['year'][0]})  ")
    lines.append(f"**Max coverage:** {coverage['pct_complete'].max():.1f}% (year {coverage.filter(pl.col('pct_complete') == pl.col('pct_complete').max())['year'][0]})  ")
